import { Controller, Get, HttpCode, Inject, NotFoundException, Param, Post, Res } from '@nestjs/common';
import { createReadStream } from 'node:fs';
import { access } from 'node:fs/promises';
import { join } from 'node:path';
import { FastifyReply } from 'fastify';
import { GameLaunchContext, GameTicket, ServerCatalogItem, ServerMod, ServerPlayer, SignedInstallManifest } from '@lapis/contracts';
...
export class ServersController {
  ...
  @HttpCode(201)
  @Post(':id/game-ticket')
  async issueTicket(@Param('id') id: string, @CurrentUser() user: AccessUser): Promise<GameTicket> {
    return this.tickets.issue(user.id, id);
  }

  @HttpCode(201)
  @Post(':id/game-launch-context')
  async issueLaunchContext(@Param('id') id: string, @CurrentUser() user: AccessUser): Promise<GameLaunchContext> {
