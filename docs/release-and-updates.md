...
- Установка блокируется, пока работает Minecraft. Пользовательские данные `%USERPROFILE%\.lapis`, игровые instances и сессия находятся вне каталога приложения и обновлением не заменяются.
...
- после публикации workflow проверяет, что публичный feed содержит новую версию.
...
git tag launcher-v0.1.6
git push origin main
git push origin launcher-v0.1.6
```
