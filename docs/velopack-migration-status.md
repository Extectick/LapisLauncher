...
## Ручной acceptance test 0.1.5 → 0.1.6

1. Установить опубликованный Setup 0.1.5, войти в аккаунт и убедиться, что профиль `%USERPROFILE%\.lapis` сохранён.
2. Опубликовать тег `launcher-v0.1.6`; workflow обязан создать Full и Delta, проверить подпись/лимиты размера и атомарно заменить feed.
3. На уже открытой 0.1.5 должна появиться надпись «Доступно обновление» и рабочее модальное окно.
4. Для startup-пути заново запустить 0.1.5: должен появиться gate, пройти delta download, launcher должен закрыться, обновиться и запуститься как 0.1.6.
...
Миграция считается production-завершённой после ручного подписанного теста 0.1.5 → 0.1.6 через реальный `lapis-mc.ru`, включая delta, startup update, already-open update, сетевую ошибку и сохранность пользовательских данных.
