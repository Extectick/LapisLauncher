...
Дата проверки: 25 августа 2026. Этот файл — контрольный список фактической реализации и оставшихся production-действий.
...
- [x] Состояние Minecraft больше не очищается только по строке `Stopping!`: лаунчер ждёт фактический выход JVM и через 15 секунд завершает зависшее дерево процесса.
- [x] Пока JVM завершает работу, повторный запуск и установка обновления остаются заблокированными; нормальный выход отменяет fallback-таймер.
- [x] Монитор читает только хвост игрового лога, а не весь растущий файл; старый `Stopping!` от предыдущего запуска не считается текущим завершением.
...
12. Закрыть Minecraft и проверить, что кнопка остаётся в состоянии остановки до фактического выхода Java; при искусственно зависшей JVM она завершается через 15 секунд, после чего повторный запуск доступен.
